package org.foxesworld.engine.utils.io;
...
public final class HOutput implements AutoCloseable, Flushable {
    ...
    public void writeByteArray(byte[] bytes, int max) throws IOException {
        if (bytes.length > max) {
            throw new IOException("Byte array size exceeds maximum allowed size.");
        }
    ...
    public void writeString(String s, int maxBytes) throws IOException {
        byte[] bytes = IOHelper.encode(s);
        if (bytes.length > maxBytes) {
            throw new IOException("String exceeds max byte size.");
        }
        writeByteArray(bytes, maxBytes);
    }
